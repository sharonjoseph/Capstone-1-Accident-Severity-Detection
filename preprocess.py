...
class prediction(object):
    ...
    def preprocess(self):
       ...
       # self.df.drop(self.df.loc[self.df['Urban_or_Rural_Area']=='Unallocated'].index, inplace=True)
        ...
        accident_severity = {"Accident_Severity": {"Serious": 1, "Fatal": 2, "Minor": 0}}
    ...
    def splitEqualRatio(self):
        ...
        XY_Minor = XY[XY["Accident_Severity"]=="Minor"]
        ...
        X_Minor = XY_Minor[self.cols]
        Y_Minor = XY_Minor[['Accident_Severity']]
        X_Minor_train, X_Minor_test, Y_Minor_train, Y_Minor_test = train_test_split(X_Minor, Y_Minor, test_size=0.25)
        ...
        X_train = X_Serious_train.append(X_Minor_train)
        X_train = X_train.append(X_Fatal_train)

        Y_train = Y_Serious_train.append(Y_Minor_train)
        Y_train = Y_train.append(Y_Fatal_train)

        X_test = X_Serious_test.append(X_Minor_test)
        X_test = X_test.append(X_Fatal_test)

        Y_test = Y_Serious_test.append(Y_Minor_test)
        Y_test = Y_test.append(Y_Fatal_test)
        return X_train, Y_train, X_test, Y_test

    def analyzeData(self):
        sns.countplot("Accident_Severity",data=self.df)
        Count_Minor_accidents = len(self.df[self.df["Accident_Severity"]=="Minor"]) # Minor accidents 
        ...
        Percentage_of_Minor_accidents = Count_Minor_accidents/(Count_Minor_accidents+Count_Serious_accidents+Count_Fatal_accidents)
        print("percentage of Minor accident is",Percentage_of_Minor_accidents*100)
        Percentage_of_Serious_accidents = Count_Serious_accidents/(Count_Minor_accidents+Count_Serious_accidents+Count_Fatal_accidents)
        print("percentage of serious accident is",Percentage_of_Serious_accidents*100)
        Percentage_of_Fatal_accidents = Count_Fatal_accidents/(Count_Minor_accidents+Count_Serious_accidents+Count_Fatal_accidents)
    ...
    def gaussianClassifier(self):
        ...
        print("accuracy_score for gaussianClassifier: %f" %(accuracy_score(Y_test, Y_pred)))
    ...
    #class_weight = dict({2:1, 1:15, 0:50})
        ...
        print("accuracy_score for RandomforestTree: %f" %(accuracy_score(Y_test, Y_pred)))
        self.printAnalysis(Y_test, Y_pred)
        return rdf
    ...
    def logisticRegression(self):
        ...
        print("accuracy_score for LogisticRegression: %f" %(accuracy_score(Y_test, Y_pred)))
        self.printAnalysis(Y_test, Y_pred)
    ...
    def __init__(self):
        ...
        filename = os.path.join(dirname, 'data/Accident_Information2.csv')
